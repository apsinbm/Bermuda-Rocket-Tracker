...
const MISSION_TRAJECTORY_DATABASE: Record<string, TrajectoryMapping> = {
  ...
  // Polar and SSO missions
  'polar-orbit': { azimuth: 45, direction: 'Northeast', confidence: 'high', source: 'database' },
  'sso-mission': { azimuth: 45, direction: 'Northeast', confidence: 'high', source: 'database' }
};

/**
 * Orbital inclination to launch azimuth mapping for Cape Canaveral
 * Based on orbital mechanics: azimuth = arcsin(cos(inclination) / cos(launch_latitude))
 */
function calculateLaunchAzimuthFromInclination(inclination: number, launchLatitude: number = 28.5): number {
  ...
  // Standard formula for launch azimuth
  const cosAzimuth = Math.cos(incRad) / Math.cos(latRad);
  
  // Handle impossible orbits (inclination < launch latitude)
  ...
  // Most missions use ascending node (eastward launch)
  // Descending node would be 180 - azimuth
...
function analyzeOrbitParameters(launch: Launch): OrbitParameters {
  ...
  if (orbitName.includes('sso') || orbitName.includes('sun-synchronous')) {
    return {
      inclination: 98, // Sun-synchronous orbit inclination
      apogee: 800,
      perigee: 800,
      orbitType: 'SSO'
    };
