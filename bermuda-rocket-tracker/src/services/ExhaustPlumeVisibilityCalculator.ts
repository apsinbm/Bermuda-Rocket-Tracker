...
export class ExhaustPlumeVisibilityCalculator {
  ...
  // Helper methods (reused from SimpleVisibilityCalculator)
  private static getTrajectoryDirection(launch: Launch): 'Northeast' | 'East-Northeast' | 'East' | 'East-Southeast' | 'Southeast' | 'Unknown' {
    ...
    // CRITICAL FIX: Override trajectory direction for X-37B missions to prevent external data corruption
    if (mission.includes('x-37b') || mission.includes('x37b') || 
      ...
      console.log(`[ExhaustPlumeCalculator] X-37B mission "${launch.mission.name}" - forcing Northeast trajectory direction`);
      return 'Northeast';
    }
